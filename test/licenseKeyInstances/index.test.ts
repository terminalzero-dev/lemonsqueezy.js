...
describe("List all license key instances", () => {
  ...
  it("Should return all license key instances with related resources", async () => {
    const {
      ...
      error,
      data: _data,
    } = await listLicenseKeyInstances({ include: ["license-key"] });
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
  ...
  it("Should return license key instances filtered by license key id", async () => {
    const {
      ...
      error,
      data: _data,
    } = await listLicenseKeyInstances({ filter: { licenseKeyId } });
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
...
describe("Retrieve a license key instance", () => {
  ...
  it("Should return a license key instance", async () => {
    const {
      ...
      error,
      data: _data,
    } = await getLicenseKeyInstance(licenseKeyInstanceId);
    ...
    const { license_key_id, identifier, name, created_at, updated_at } =
      attributes;
