...
describe("Create a usage record", async () => {
  it("Should return a usage record object using the subscription item id & quantity", async () => {
    ...
    const { subscription_item_id, quantity, action, created_at, updated_at } =
      attributes;
    const items = [
...
describe("List all usage records", () => {
  ...
  it("Should return a paginated list of usage records with related resources", async () => {
    const {
      ...
      statusCode,
      data: _data,
    } = await listUsageRecords({ include: ["subscription-item"] });
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
  ...
  it("Should return a paginated list of usage records filtered by subscription item id", async () => {
    const {
      ...
      statusCode,
      data: _data,
    } = await listUsageRecords({ filter: { subscriptionItemId } });
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
    ...
    expect(
      data.filter(
        (item) =>
          item.attributes.subscription_item_id === Number(subscriptionItemId)
      ).length
    ).toEqual(data.length);
...
describe("Retrieve a usage record", () => {
  ...
  it("Should return a usage record object", async () => {
    const {
      ...
      statusCode,
      data: _data,
    } = await getUsageRecord(usageRecordId);
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
  ...
  it("Should return a usage record object with related resources", async () => {
    const {
      ...
      statusCode,
      data: _data,
    } = await getUsageRecord(usageRecordId, { include: ["subscription-item"] });
    expect(statusCode).toEqual(200);
    expect(error).toBeNull();
    expect(_data).toBeDefined();
