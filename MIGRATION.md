...
| Webhook identifier segments       | Raw Webhook IDs were interpolated into paths, so URL syntax inside an ID could change the requested path.                      | Webhook IDs are treated as one raw path segment and percent-encoded once.                                    | Pass raw, unencoded IDs; remove manual URL encoding and update request or proxy assertions.                                   |
| Webhook subscription inputs       | Runtime accepted empty or unsupported event arrays, while declarations omitted newer reviewed events.                          | Create and update accept the reviewed closed event set and reject invalid input before transport.            | Compile against `WebhookSubscriptionEventName`, remove event-name casts, and verify every configured subscription.            |
| Webhook secrets in errors         | API, parsing, abort, or network details could expose a Webhook signing secret to logs and observers.                           | Known signing-secret values are redacted while error status and diagnostic structure are kept.               | Remove logging or tests that depend on raw secret values and verify downstream logs stay redacted.                            |
| License API request protocol      | License calls used JSON and JSON:API media types instead of the documented form protocol.                                      | Calls send only operation-defined form fields, accept JSON, and never send Bearer authorization.             | Update request assertions or proxies and keep License Keys separate from API credentials.                                     |
| License request controls          | License calls bypassed the shared SDK timeout and did not support caller cancellation.                                         | License calls use the shared one-attempt timeout and cancellation behavior behind Resource Runtime.          | Verify timeout expectations and handle `aborted` and `timeout` errors without retrying mutations.                             |
| License failure secrecy           | HTTP, parsing, network, abort, timeout, Compatibility envelope, or Error observer diagnostics could expose License Key values. | Submitted License Key and instance values are redacted from failures while successful bodies stay unchanged. | Remove logging or tests that depend on raw values and verify business-negative success handling separately.                   |
...
- **Webhook identifier segments:** [Operation Contract](./src/namespaces/webhooks/contract.ts) and [Namespace fixture](./src/namespaces/webhooks/webhooks.test.ts).
- **Webhook subscription inputs:** [closed event type](./src/namespaces/webhooks/types.ts) and [Namespace fixture](./src/namespaces/webhooks/webhooks.test.ts).
- **Webhook secrets in errors:** [redaction implementation](./src/internal/v5/redaction.ts) and [Namespace fixture](./src/namespaces/webhooks/webhooks.test.ts).

License audit evidence:

- **License API request protocol:** [Operation Contract](./src/namespaces/license/contract.ts) and [Namespace fixture](./src/namespaces/license/license.test.ts).
- **License request controls:** [HTTP Core](./src/internal/v5/http.ts) and [Namespace fixture](./src/namespaces/license/license.test.ts).
- **License failure secrecy:** [redaction implementation](./src/internal/v5/redaction.ts), [Namespace fixture](./src/namespaces/license/license.test.ts), and [Compatibility fixture](./test/v5/compat-license-api.test.ts).
